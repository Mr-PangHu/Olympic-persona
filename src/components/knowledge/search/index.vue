            ...
            <div class="content">
              <el-form :inline="true" :model="formInline" :rules="rules" ref="ruleForm" class="demo-form-inline">
                <el-form-item label="第一大类" prop="firstClass">
                  <el-select v-model="formInline.firstClass" placeholder="选择第一大类" filterable clearable
                    @change="getSecondClass" @clear="clearFirst" @focus="focusFirst">
                    <el-option v-for="(item,index) in firstClass" :key="index" :label="item" :value="item"></el-option>
                  </el-select>
                </el-form-item>
                <el-form-item label="第二大类" prop="secondClass">
                  <el-select v-model="formInline.secondClass" @change="getFoodName" @clear="clearSecond"
                    @focus="focusSecond" filterable clearable placeholder="选择第二大类">
                    <el-option v-for="(item,index) in secondClass" :key="index" :label="item" :value="item"></el-option>
                  </el-select>
                </el-form-item>
                <el-form-item label="食物名字" prop="foodId">
                  <el-select v-model="formInline.foodId" filterable clearable placeholder="选择食物名字">
                    <el-option v-for="item in food" :key="item.id" :label="item.foodName" :value="item.id"></el-option>
                  </el-select>
                </el-form-item>
                <el-form-item>
                  <el-button type="primary" icon="el-icon-search" @click="handleClick('ruleForm')"
                    style="background-color: #00254FCC;border-color: #00254FCC;">查询</el-button>
                </el-form-item>
              </el-form>
            </div>
          </div>
